...
from parse import connectin_check_time, get_data_from_json_for_calendar
...
async def process_start_command(message: types.Message, state: FSMContext):
    ...
    with open('photos/1.png', 'rb') as photo:
        await message.answer_photo(photo, caption='Здравствуйте! Создайте новую интеграцию по ссылке:',
                                   reply_markup=markup)
    with open('photos/2.png', 'rb') as photo:
        await message.answer_photo(photo, caption='Скопировуйте notion token.')
    await message.answer("Введите notion token:")
    await Form.waiting_token.set()


@dp.message_handler(state=Form.waiting_token)
async def process_name(message: types.Message, state: FSMContext):
    # if not os.path.exists('data/all_tokens.json'):
    #     with open('data/all_tokens.json', 'w') as file:
    #         json.dump({}, file)
    #
    # with open('data/all_tokens.json') as file:
    #     all_tokens = json.load(file)
    user_token = message.text
    # all_tokens[str(message['from']['id'])] = {'NotionToken': user_token}
    # with open('data/all_tokens.json', 'w') as file:
    #     json.dump(all_tokens, file, ensure_ascii=False)
    conn = sqlite3.connect("data/tokens.sql")
    cursor = conn.cursor()
    cursor.execute(
    ...
    with open('photos/4.png', 'rb') as photo:
        await message.answer_photo(photo, caption="Нажмите Share и скопируйте ссылку. Это и будет database token")
    # with open('photos/5.png', 'rb') as photo:
    #     await message.answer_photo(photo,
    #                                caption="Скопируйте только выделеную часть от / до ?. Это и будет database token.")
    await message.answer(f"Введите notion database token:")
...
async def process_token(message: types.Message, state: FSMContext):
    ...
    # all_tokens[str(message['from']['id'])]['DBToken'] = user_token
    # with open('data/all_tokens.json', 'w') as file:
    #     json.dump(all_tokens, file, ensure_ascii=False)
    conn = sqlite3.connect("data/tokens.sql")
    cursor = conn.cursor()
...
def calender_buttons(today):
    ...
    for week in weeks:
        ...
        for day in week:
            number = day[0]
            button = types.InlineKeyboardButton(str(number), callback_data=f'exdt{str(today)[:8]}{str(number)}')
...
async def callback(call):
    ...
    if 'prev' in call.data:
        ...
        await call.message.answer(f"Выбранная дата: {months[today.month - 1]} {today.year} Года", reply_markup=markup)
    ...
    if len(str(call.data)) == 2:
        if connectin_check_time(dict(call).get('from').get('id')):
            all_dd = test_function(answer_deadline_filter(call.data, f'data/events/{UserID}processed_parse2'))
            for dd in all_dd:
                await call.message.answer(dd)
        else:
            await call.message.answer('Что-то пошло не так! Попробуйте ввести токен заново! Нажмите /start')
...
async def show_calendar(message):
    today = date.today()
    markup = calender_buttons(today)
    await message.answer(f"Выбранная дата: {months[today.month - 1]} {today.year} Года", reply_markup=markup)
